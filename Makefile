...
UCI_OBJ_DIR = $(OBJ_DIR)/uci_mode
UCI_CFLAGS = $(CFLAGS) -DUCI_MODE_MAIN
...
UCI_MODE_SRC_FILES = $(SRC_DIR)/board.c $(SRC_DIR)/game.c $(SRC_DIR)/move.c $(SRC_DIR)/attack_data.c $(SRC_DIR)/move_gen.c $(SRC_DIR)/search.c $(SRC_DIR)/transposition_table.c $(SRC_DIR)/uci_adapter.c
...
UCI_MODE_OBJ_FILES = $(patsubst %.c, $(UCI_OBJ_DIR)/%.o, $(notdir $(UCI_MODE_SRC_FILES)))
...
$(UCI_OBJ_DIR)/%.o: $(SRC_DIR)/%.c | $(UCI_OBJ_DIR)
	$(CC) $(UCI_CFLAGS) -c $< -o $@
...
uci_mode.exe: $(UCI_MODE_OBJ_FILES)
	$(CC) -o $@ $^

# Clean
clean:
	rm -rf $(OBJ_DIR) springer_tests springer search_benchmarks uci_mode.exe springer_tests.exe springer.exe search_benchmarks.exe