# Build
Requires GCC.
- `make game` builds `springer.exe`.
- `make tests` builds `springer_tests.exe`
- `make clean` removes the `obj` directory and built executables.

# Development
Currently, full move generation is complete, including castling, en passant and pawn promotion. Endgame scenarios for checkmate, stalemate, draw by fifty-move rule, and draw by insufficient material are complete. Search is yet to be implemented.


<img width="405" height="582" alt="image" src="https://github.com/user-attachments/assets/7468de5c-5c56-4273-abd0-cbc3d34231db" />
